...
export const useGetExpenses = ({onSuccess,onError,userId,IdToken}:{onSuccess:({data}:{data:spending[]})=>void,onError: ({response}:{response:any}) => void, userId:string, IdToken:string}) => {
      return useQuery({
            queryKey: ['expenses'],
            queryFn: async () => getExpenses({userId, IdToken}),
